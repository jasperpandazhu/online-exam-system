	...
	styleUI : {
		...
		Bootstrap : {
			...
			navigator : {
				...
				icon_view_nav : "glyphicon-system-sign",
		...
		Bootstrap4 : {
			...
			base : {
				entrieBox : "",
				viewBox : "table-responsive",
				headerTable : "table table-bordered table-condensed",
				headerBox : "",
				rowTable : "table table-bordered table-condensed",
				rowBox : "",
				stripedTable : "table-striped",
				footerTable : "table table-bordered table-condensed",
				footerBox : "",
				headerDiv : "",
				gridtitleBox : "",
				customtoolbarBox : "",
				...
				multiBox : "checkbox",
				pagerBox : "",
				pagerTable : "table table-condensed",
				toppagerBox : "",
				pgInput : "form-control",
				pgSelectBox : "form-control",
				pgButtonBox : ""
	...
	return this.each( function() {
		...
		updatepager = function(rn, dnd) {
			...
			if(pgboxes) {
				...
				if (ts.p.viewrecords){
					if(ts.p.reccount === 0) {
						$(".ui-paging-system",pgboxes).html($.jgrid.getRegional(ts, "defaults.emptyrecords", ts.p.emptyrecords ));
					} else {
						from = base+1;
						tot=ts.p.records;
						if($.fmatter) {
						...
						$(".ui-paging-system",pgboxes).html($.jgrid.template( rt ,from,to,tot));
					}
				}
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.viewrecords===true) {
				$("td#"+pgid+"_"+ts.p.recordpos,"#"+pgcnt).append("<div dir='"+dir+"' style='text-align:"+ts.p.recordpos+"' class='ui-paging-system'></div>");
			}
...
$.jgrid.extend({
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			if(rp_ge[$t.p.id].checkOnSubmit || rp_ge[$t.p.id].checkOnUpdate) {
				bS  ="<a id='sNew' class='fm-button btn-sm "+commonstyle.button + "' style='z-index:1002'>"+p.bYes+"</a>";
				bN  ="<a id='nNew' class='fm-button btn-sm "+commonstyle.button + "' style='z-index:1002;margin-left:5px'>"+p.bNo+"</a>";
				bC  ="<a id='cNew' class='fm-button btn-sm "+commonstyle.button + "' style='z-index:1002;margin-left:5px;'>"+p.bExit+"</a>";
...
/*
    ...
    More system: https://pixabay.com/blog/posts/p-52/
